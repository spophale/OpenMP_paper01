...
(A similar concept~\cite{nikolopoulos2001exploiting} has been proposed as a schedule in the OpenMP \textbf{parallel for} construct  as a proposed extension).
...
work-sharing constructs. OpenMP 4.5 also provides a mechanism to map data to and from accelerators using the target data regions constructs. The new OpenMP 4.5 release provides a substantial 
...
The POWER8 system has a Non-Uniform Cache Architecture (NUCA) Cache Policy, this allows for scalable bandwidth and latency, allowing migration of most used cache lines to the local L2 cache and then to the local L3 cache~\cite{IBM_P8}. This is a big improvement over the POWER7 processor.  

When we run the command  \textit{numactl} on a two socket POWER8 system, it  reports four NUMA domains, one for each POWER8 chiplet. 
The utility also reports the NUMA distances between domains, which is the ratio of the latency of accessing  a remote numa node memory and local memory access. Below are the values of different NUMA distances as reported by \textit{numactl} (see ~\ref{fig:crest}): 
